...
#else
#if (FLASH_SIZE == 16)
#define MTDPARTS_DEFAULT    "mtdparts=spi0.0:256k(u-boot),256k(u-boot-env),1280k(custom),1024k(kernel),6144k(rootfs),7168k(inactive),256k(ART)"
#elif (FLASH_SIZE == 8)
...
#define	CONFIG_BOOTARGS     "console=ttyATH0,115200 rootfstype=squashfs,jffs2 init=/etc/preinit board="BOARD_NAME
...
"bootcmd_2=run imagechk && bootm 0x9f8c0000\0" \
"set_bootargs_1=set bootargs_1 ${bootargs} root=31:04 mtdparts=spi0.0:256k(u-boot),256k(u-boot-env),1280k(custom),${kernel_size_1}k(kernel),${rootfs_size_1}k(rootfs),7168k(inactive),256k(ART)\0" \
"set_bootargs_2=set bootargs_2 ${bootargs} root=31:05 mtdparts=spi0.0:256k(u-boot),256k(u-boot-env),1280k(custom),7168k(inactive),${kernel_size_2}k(kernel),${rootfs_size_2}k(rootfs),256k(ART)\0" \
"kernel_size_1=1024\0" \
"rootfs_size_1=6144\0" \
"kernel_size_2=1024\0" \
"rootfs_size_2=6144\0"
#elif (FLASH_SIZE == 8)
#define CONFIG_EXTRA_ENV_SETTINGS \
"preboot=flashit 0x80100000 fwupgrade.cfg\0" \
"imagechk=test -n \"${check_skip}\" || check_skip=1 && datachk vmlinux,rootfs\0" \
